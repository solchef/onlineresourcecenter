                            ...
                            <tbody>
                                @foreach ($assignments as $ass)
                                    <tr>
                                        ...
                                        <td>
                                            @if ($ass->status === 'unlocked')
                                                <a href="{{ asset('assignments') }}/{{ $ass->uploadassignment }}"
                                                    download><button class="btn btn-xs btn-success col-md-12"><i
                                                            class="fa fa-download"> @lang('Download')</i></button>
                                                </a>
                                            @else
                                                <a href="#modal">
                                                    <button class="btn btn-xs btn-warning col-md-12"><i class="fa fa-lock">
                                                            @lang('Download Locked')</i></button>
                                                </a>
                                            @endif
                                        ...
                                        @if ($ass->status === 'locked')
                                            <td>
                                                <a href="#modal">
                                                    <button class="btn btn-xs btn-warning col-md-12"><i class="fa fa-lock">
                                                            @lang('Submission Locked')</i></button>
                                            </td>
                                            </a>
                                        @else
                                            @if ($today < $ass->submitby)
                                                <td><a href="{{ url('submitassignment') }}/{{ $ass->id }}"><button
                                                            class="btn btn-xs btn-success col-md-12"><i
                                                                class="fa fa-code-fork">
                                                                @lang('Submit')</i></button></a></td>
                                            @else
                                                <td><a href="{{ url('submitassignment') }}/{{ $ass->id }}"><button
                                                            class="btn btn-xs btn-success col-md-12"><i
                                                                class="fa fa-code-fork">
